...
blogsRouter.post('/', async (request, response) => {
  ...
  try {
    ...
    response.status(201).json({ ...body, id: savedBlog._id, likes: 0, comments: [], user: { id: user.id, name: user.name, username: user.username }})
  } catch (error) {
...
blogsRouter.post('/:id/comments', async (request, response) => {
  try {
    const { blog, comment } = request.body
    if (comment.trim() === '') {
      response.status(400).json({ error: "Can't be an empty comment" })
    }
    const updatedBlog = await Blog.findByIdAndUpdate(blog.id, { ...blog }, { new: true, runValidators: true, context: 'query' })
    if (!updatedBlog) response.status(404).json({ error: "blog not found" })
